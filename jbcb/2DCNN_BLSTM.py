...
from keras.layers import Input, Embedding, LSTM, Dense, merge, Convolution2D, TimeDistributedDense, Reshape,MaxPooling2D,Convolution1D,BatchNormalization
...
m_output = MaxPooling2D(pool_size=(1, 2), strides=None, border_mode='same')(a)
print 'm_output.getshape()',m_output.get_shape()
...
f1 = LSTM(output_dim=200,return_sequences=True, activation='tanh', inner_activation='sigmoid',dropout_W=0.5,dropout_U=0.5)(d_output)
f2 = LSTM(output_dim=200, return_sequences=True, activation='tanh', inner_activation='sigmoid', go_backwards=True,dropout_W=0.5,dropout_U=0.5)(d_output)
# f = merge([d,e], mode='sum',concat_axis=2)
# f=Dropout(0.5)(f)
f3 = LSTM(output_dim=200, return_sequences=True, activation='tanh', inner_activation='sigmoid', dropout_W=0.5,dropout_U=0.5)(f1)
f4 = LSTM(output_dim=200, return_sequences=True, activation='tanh', inner_activation='sigmoid', go_backwards=True,dropout_W=0.5,dropout_U=0.5)(f2)
...
load_file = "./model/2cnn(3(21)42-200-0.003-400-600)-de-2LSTM-CB513.h5"
checkpointer = ModelCheckpoint(filepath=load_file,verbose=1,save_best_only=True)
history=model.fit({'main_input': train_hot, 'aux_input': trainpssm},
          {'main_output': trainlabel},validation_data=({'main_input': val_hot, 'aux_input': valpssm},{'main_output': vallabel}),
